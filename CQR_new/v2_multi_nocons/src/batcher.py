...
class Example(object):
  ...
  def __init__(self, article, labeling, abstract_sentences, vocab, hps):
...
class Batch(object):
  ...
  def init_encoder_seq(self, example_list, hps):
    ...
    self.labeling_batch = np.zeros((hps.batch_size.value, max_enc_seq_len), dtype=np.int32)
  ...
  def init_decoder_seq(self, example_list, hps):
    ...
    len1 = len(example_list[0].dec_input)
    len2 = len(example_list[0].target)
    self.dec_batch = np.zeros((hps.batch_size.value, len1), dtype=np.int32)
    self.target_batch = np.zeros((hps.batch_size.value, len2), dtype=np.int32)
    self.dec_padding_mask = np.zeros((hps.batch_size.value, len1), dtype=np.float32)
...
class Batcher(object):
  ...
  def fill_example_queue(self):
    ...
    while True:
      try:
        (article, labeling, abstract) = next(input_gen) # read the next example from file. article and abstract are both strings.
      ...
      example = Example(article, labeling, abstract_sentences, self._vocab, self._hps) # Process into an Example.
  ...
  def text_generator(self, example_generator):
    ...
    while True:
      ...
      try:
        ...
        labeling = e.features.feature['labeling'].bytes_list.value[0].decode()
      except ValueError:
        tf.logging.error('Failed to get data from example')
        continue
      if not article_text or (not abstract_text) or (not labeling): # See https://github.com/abisee/pointer-generator/issues/1
        tf.logging.warning('Found an example sample. Skipping it.')
      else:
        yield (article_text, labeling, abstract_text)
