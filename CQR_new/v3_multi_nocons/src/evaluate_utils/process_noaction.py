...
ratio = "all"
decoded = codecs.open("../log_old/" + ratio + "/decode/dec.txt").readlines()
reference = codecs.open("../log_old/" + ratio + "/decode/ref.txt").readlines()
...
for idx, sec in enumerate(decoded):
    ...
    if pred == ref:
        ...
        decoded_final.append(raw.split("||")[0].split(";")[-1].strip())
