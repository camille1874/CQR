...
class SummarizationModel(object):
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq'):
      ...
      self.num_steps = 100
      self.use_crf = False
      self.labeling_outputs = tf.reshape(tf.concat(enc_outputs, 1), [-1, hps.hidden_dim.value * 2])
      softmax_w = tf.get_variable("softmax_w", [hps.hidden_dim.value * 2, self.num_classes]) # BI0
      ...
      if not self.use_crf:
        ...
        self.tags_scores = tf.reshape(self.labeling_outputs, [hps.batch_size.value, self.num_steps, self.num_classes]) # [hps.batch_size.value, self.num_steps, self.num_classes]
        self.transitions = tf.get_variable("transitions", [self.num_classes + 1, self.num_classes + 1])

        dummy_val = -1000
        class_pad = tf.Variable(dummy_val * np.ones((hps.batch_size.value, self.num_steps, 1)), dtype=tf.float32)
        self.observations = tf.concat([self.tags_scores, class_pad], 2)

        begin_vec = tf.Variable(np.array([[dummy_val] * self.num_classes + [0] for _ in range(hps.batch_size.value)]), trainable=False, dtype=tf.float32)
        end_vec = tf.Variable(np.array([[0] + [dummy_val] * self.num_classes for _ in range(hps.batch_size.value)]), trainable=False, dtype=tf.float32)
        begin_vec = tf.reshape(begin_vec, [hps.batch_size.value, 1, self.num_classes + 1])
        end_vec = tf.reshape(end_vec, [hps.batch_size.value, 1, self.num_classes + 1])

        self.observations = tf.concat([begin_vec, self.observations, end_vec], 1)

        self.mask = tf.cast(tf.reshape(tf.sign(self.targets), [hps.batch_size.value * self.num_steps]), tf.float32)

        # point score
        self.point_score = tf.gather(tf.reshape(self.tags_scores, [-1]),
                                     tf.range(0, hps.batch_size.value * self.num_steps) * self.num_classes + tf.reshape(
                                       self.targets, [hps.batch_size.value * self.num_steps]))
        ...
        # all path score
        self.total_path_score, self.max_scores, self.max_scores_pre = self.forward(self.observations, self.transitions,
                                                                                   self.length)
        # loss
        self.labeling_loss = - (self.target_path_score - self.total_path_score)
      ...
      if hps.mode.value in ['train', 'eval']:
        ...
        with tf.variable_scope('loss'):
          if FLAGS.pointer_gen:
            ...
            self._loss = _mask_and_avg(loss_per_step, self._dec_padding_mask) + self.labeling_loss
