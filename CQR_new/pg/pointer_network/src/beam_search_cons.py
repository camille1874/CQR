...
def run_beam_search(sess, model, vocab, batch, question):
  ...
  while steps < 50 and len(results) < FLAGS.beam_size:
    ...
    for h in sort_hyps(all_hyps, question, batch, vocab): # in order of most likely h
  ...
  hyps_sorted = sort_hyps(results, question, batch, vocab)

  # Return the hypothesis with highest average log prob
  return hyps_sorted[0]

def get_score(result, question):
  score = 1
  ...
  if not items:
    ...
    if not res_en:
      ...
      idx = []
      for e in res_en:                                                                                      idx.append(e[e.find("entity") + 6: e.find(".")])
...
def sort_hyps(hyps, question, batch, vocab):
  ...
  for h in hyps:
    ...
    try:
      ...
      result = result
    result = " ".join(result)
    score = get_score(result, question)
