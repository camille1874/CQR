...
for idx, sec in enumerate(decoded[:199787]):
    ...
    if "replace" in cls:
        if "ref" in cls:
            coref_ref.append(ref) 
            coref_dec.append(pred)   
        elif "and" in cls or "or" in cls or "not" in cls:
            logical_ref.append(ref)
            logical_dec.append(pred)
        else:
            incomp_ref.append(ref)
            incomp_dec.append(pred)   
    elif "norewrite" in cls:
        if len(raw.split("||")[0].split(";")) == 1:
            no_single_ref.append(ref)
            no_single_dec.append(pred)
        else:
            no_context_ref.append(ref)
            no_context_dec.append(pred)
